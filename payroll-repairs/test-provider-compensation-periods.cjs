...
test('retained hygienist flow queries Sep18 Aug30–Sep12 while doctors use the Ledger snapshot',async()=>{const h=dataHarness(),p=h.load();h.pending[0].resolve(data(123.45));await p;assert.equal(h.calls[0].startDate,'2026-08-30');assert.equal(h.calls[0].endDate,'2026-09-12');assert.equal(h.calls[0].locationId,'office-one');assert.equal(h.calls[0].requireComplete,true);assert.equal(h.state.Providers[0].providerId,'provider-one');assert.equal(h.state.Providers[0].collections,123.45);assert.equal(h.state.Providers[0].monthlyCollections,1000);});
