...
expected='dec9768c7d981725d965c1c0fff6f47de8ec8c10ca41fd2ef14a80434a6a0f4a';assert hashlib.sha256(before).hexdigest()==expected and (folder/'main.py').resolve()==source.resolve()
...
old='        if comparisonMode in ("daily", "mtd") and not loc_id:\n            by_office = []\n            for _lid, _lname in LOCATION_NAMES.items():\n'
new='        if comparisonMode in ("daily", "mtd"):\n            by_office = []\n            for _lid, _lname in LOCATION_NAMES.items():\n                if loc_id and _lid != loc_id:\n                    continue\n'
text=before.decode();assert text.count(old)==1;after=text.replace(old,new,1).encode();assert after.replace(new.encode(),old.encode(),1)==before;compile(after,'main_candidate.py','exec')
dest=root/'ndash076-backend';dest.mkdir(mode=0o700,exist_ok=False);(dest/'main_candidate.before.py').write_bytes(before);candidate=dest/'main_candidate.py';candidate.write_bytes(after)
...
def run(label,script,env_extra=None,args=None,expected_failure=False):
    ...
    if expected_failure:assert result.returncode!=0 and 'failures=2' in result.stderr,'Expected two single-office failures before repair'
...
run('before-office','phase4-test-office-breakdown.py',{'NDASH_OFFICE_SOURCE':str(dest/'main_candidate.before.py')},expected_failure=True)
run('candidate-office','phase4-test-office-breakdown.py',{'NDASH_OFFICE_SOURCE':str(candidate)})
...
out={'issue':'NDASH-076','before_main_sha256':expected,'candidate_main_sha256':hashlib.sha256(after).hexdigest(),'patch':{'old':old,'new':new},'syntax':'PASS','full_reversal':'PASS','checks':checks,'production_unchanged':True}
p=root/'ndash076-backend-candidate.json';p.write_text(json.dumps(out,indent=2));p.chmod(0o600);print(json.dumps({k:v for k,v in out.items() if k!='patch'}))
