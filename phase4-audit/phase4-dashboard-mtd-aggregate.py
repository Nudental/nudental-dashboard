...
import ast,hashlib,json,pathlib,urllib.request,urllib.parse,sys
...
date=sys.argv[1] if len(sys.argv)>1 else '2026-08-31';assert date in ['2026-08-31','2026-12-31']
params={'date':date,'officeId':'b0abcc46-55e8-4529-a28f-eedf41c1d72e','comparisonMode':'mtd','comparisonYears':1,'page':1}
...
mtd=data['mtd']['current_mtd'];out={'office':'Staten Island','report_date':date,'net_production':mtd.get('net_production'),'collections':mtd.get('total_collections'),'read_only':True,'raw_identity_or_record_content_exposed':False};assert isinstance(out['net_production'],(int,float)) and isinstance(out['collections'],(int,float));print(json.dumps(out))
