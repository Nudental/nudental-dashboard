"""Release the inclusive aging boundaries through the existing candidate/live services."""
import ast,hashlib,json,os,pathlib,stat,subprocess,time,urllib.request,urllib.parse
root=pathlib.Path(__file__).parent;folder=pathlib.Path('/home/openclaw/.openclaw/workspace/ascend_api/middleware');dest=root/'ndash062-backend';meta=json.loads((root/'ndash062-backend-candidate.json').read_text());source=folder/'main_candidate.py';before=(dest/'main_candidate.before.py').read_bytes();after=(dest/'main_candidate.py').read_bytes()
assert hashlib.sha256(before).hexdigest()=='64bb98858a7fa661a535c189f0520b312cbf55800849f0e2ac776b6c0bc3a1a5'
assert hashlib.sha256(after).hexdigest()=='8117c4026155dccb1e08d9db5e0978df2972c5b9f75850d4f1a25881e64b4ed4'
assert meta['candidate_main_sha256']==hashlib.sha256(after).hexdigest() and meta['syntax']=='PASS' and meta['full_reversal']=='PASS'
assert len(meta['checks'])==9 and all(c['result'].endswith('PASS') for c in meta['checks'][1:])
...
def verify(base):
 ...
 return {'aging':verify_aging(base),'unknown_zero':True,'mixed_status_union':True,'known_filters_unchanged':True,'production_collections_unchanged':True}
