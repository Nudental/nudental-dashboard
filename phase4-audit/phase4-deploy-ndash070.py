"""Release the documentation queue note projection through the existing candidate/live services."""
import ast,hashlib,json,os,pathlib,stat,subprocess,time,urllib.request,urllib.parse,urllib.error
root=pathlib.Path(__file__).parent;folder=pathlib.Path('/home/openclaw/.openclaw/workspace/ascend_api/middleware');dest=root/'ndash070-backend';meta=json.loads((root/'ndash070-backend-candidate.json').read_text());source=folder/'main_candidate.py';before=(dest/'main_candidate.before.py').read_bytes();after=(dest/'main_candidate.py').read_bytes()
assert hashlib.sha256(before).hexdigest()=='f37a12859a418ce4600212cde671ed9acc12680e36dbdd880d0a234627355a51'
assert hashlib.sha256(after).hexdigest()=='dec9768c7d981725d965c1c0fff6f47de8ec8c10ca41fd2ef14a80434a6a0f4a'
assert meta['candidate_main_sha256']==hashlib.sha256(after).hexdigest() and meta['syntax']=='PASS' and meta['full_reversal']=='PASS'
assert len(meta['checks'])==11 and all(c['result'].endswith('PASS') for c in meta['checks'][1:])
...
assert pos_access_status('https://api.nudashboard.com')==401 and pos_access_status('https://api.nudashboard.com',True)==401
...
record={'issue':'NDASH-070','stage':'validated','before':hashlib.sha256(before).hexdigest(),'after':hashlib.sha256(after).hexdigest(),'business_data_changes':False,'configuration_changes':False,'existing_read_only_cache_warming_preserved':True};state=root/'ndash070-backend-deployment-result.json';temporary=folder/'.main_candidate.ndash070.tmp';assert not temporary.exists();mode=stat.S_IMODE(source.stat().st_mode)
