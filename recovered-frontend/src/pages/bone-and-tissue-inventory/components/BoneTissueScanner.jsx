...
import { receiveStockByScan, consumeStockByScan, lookupStockByLotOrId } from '../../../services/boneTissueService';

const SCANNER_ID = 'bt-scanner-region';
...
const parseScannedData = (raw) => {
  const result = { identificationNumber: raw, lotNumber: '', expirationDate: '', productName: '' };
  try {
    ...
    if (parsed?.name || parsed?.product_name) result.productName = parsed?.name || parsed?.product_name;
    return result;
  } catch (_) {}
...
const BoneTissueScanner = ({ mode, offices, providers, staff, userId, userName, onClose, onSaved }) => {
  // mode: 'receive' | 'consume'
  const [step, setStep] = useState('scan'); // 'scan' | 'manual' | 'form'
  ...
  const handleScannedResult = async (parsed) => {
    setLookingUp(true);
    try {
      const match = await lookupStockByLotOrId(parsed?.identificationNumber, parsed?.lotNumber);
      setStockMatch(match);
      setForm(f => ({
        ...f,
        ...
        bone_tissue_type: match?.bone_tissue_type || '',
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === 'receive') {
        ...
        setToast(`✓ Received ${form?.quantity} unit(s) of ${form?.product_name || form?.identification_number}`);
      } else {
        await consumeStockByScan({
          officeId: form?.office_id,
          officeName: form?.office_name,
        ...
        <div className={`flex items-center justify-between px-5 py-4 ${headerBgClass}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/70">Bone &amp; Tissue Inventory</p>
            </div>
          </div>
            ...
            <div className="p-5 space-y-4">
              ...
              {stockMatch && (
                <div className={`p-3 ${alertBgClass} border ${alertBorderClass} rounded-xl flex items-start gap-3`}>
                  ...
                  <div>
                    <p className={`text-xs font-semibold ${alertTextDarkClass}`}>Stock record found</p>
                    <p className="text-xs text-muted-foreground">
                      {stockMatch?.product_name} · {stockMatch?.bone_tissue_type} · Current stock: {stockMatch?.current_stock ?? '—'}
                    </p>
              ...
              {form?.expiration_date && (() => {
                ...
                if (daysLeft < 0) return (
                  <div className="p-3 bg-red-50 border border-red-200 rounded-xl flex items-center gap-2">
                    ...
                    <p className="text-xs text-red-700 font-semibold">This item is EXPIRED ({Math.abs(daysLeft)} days ago)</p>
                  </div>
                );
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Product Name" error={undefined}>
                  <input
                    className={inputCls()}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Bone / Tissue Type" error={undefined}>
                  <select
                    className={inputCls()}
                  ...
                  >
                    ...
                    {['Bone', 'Tissue', 'Membrane', 'PRF', 'Allograft', 'Xenograft', 'Synthetic', 'Other']?.map(t => (
          ...
          <div className="px-5 py-4 border-t border-border bg-muted/30 flex items-center gap-3">
            ...
            >
              {saving ? (
                ...
                <><Icon name={modeIcon} size={14} /> {isReceive ? 'Receive Stock' : 'Log Consumption'}</>
              )}
            </button>
          </div>
        )}
...
export default BoneTissueScanner;
