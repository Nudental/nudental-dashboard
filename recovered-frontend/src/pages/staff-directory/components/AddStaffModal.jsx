...
const AddStaffModal = ({ onClose, onSave }) => {
  ...
  const validate = () => {
    ...
    if (!form?.role_category?.trim()) return 'Role category is required.';
    if (!form?.worker_type?.trim()) return 'Worker type is required.';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(ex?.message || 'Failed to add staff member.');
    } finally {
      setSaving(false);
    }
  };
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Full Name" required>
                <input className={inputCls} value={form?.full_name} onChange={e => set('full_name', e?.target?.value)} placeholder="e.g. Jane Smith" />
              </Field>
              <Field label="Display Name">
                <input className={inputCls} value={form?.directory_display_name} onChange={e => set('directory_display_name', e?.target?.value)} placeholder="Preferred display name (or same as full name)" />
              </Field>
