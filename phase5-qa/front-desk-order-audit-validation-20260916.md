...
Live bulk follow-up: two additional labeled zero-dollar fixtures (one Pending, one Pending Fulfillment) were selected together. Cancel preserved their exact rows and zero audits. Confirm closed both, immediately cleared pending actions and recorded exactly one correct before/after audit per order. Full refresh retained both Closed statuses; quantities and Not Received were unchanged. Exact-ID/value-guarded cleanup removed only these two fixtures, preserved both audits, and a repeated delete returned zero rows. UI then showed only the original single order again. Receipt: `qa-frontdesk-amazon-bulk-20260916.json`, cleanup complete.

Concurrent stale-tab replay is not claimed by these checks. The original record is retained for pending access candidate 034 and later exact-ID cleanup. Candidates 032 and 034 remain unapplied; their live permission verification is still pending explicit approval.
