...
const fn=find(parser.parse(source,{sourceType:'module',plugins:['jsx']}),n=>n.type==='VariableDeclarator'&&n.id.name==='handleBulkApprove').init;
...
function setup(statuses,{reviewed=statuses,denied=false,selection}={}) {
 ...
 state.run=vm.runInNewContext(code,{supabase,entries,selectedIds:selection||entries.map(e=>e.id),
  userProfile:{id:'qa-regional',full_name:'QA Regional',role:'regional_manager'},
...
for(const current of ['approved','rejected','pending_reapproval','rejected_after_approval'])test('bulk approval skips stale '+current+' row and accurately reports one change',async()=>{
 ...
 assert.equal(s.success.length,1);assert.match(s.success[0][1],/^1 EOD report approved\. 1 skipped/);assert.equal(s.errors.length,0);
});
test('a fully stale selection adds no history and reports no successful approvals',async()=>{
...
for(const status of ['pending','pending_review','pending_reapproval'])test('bulk approval accepts reviewed '+status+' and records accurate history',async()=>{
 const s=setup([status,status]);await s.run();assert.equal(s.writes,2);assert.equal(s.history.length,2);
 assert.ok(s.history.every(h=>h.fromStatus===status&&h.toStatus==='approved'&&h.eventType==='bulk_approval'));
 assert.equal(s.success.length,1);assert.match(s.success[0][1],/^2 EOD reports approved\./);
});
