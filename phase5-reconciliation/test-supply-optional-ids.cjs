...
test('production save retains the existing database path',async()=>{const h=harness(false,false);await h.save({office_id:'Nu Dental of Brick'},[{department_id:'',subsection_id:'',item_id:'',custom_item_name:'fixture'}]);assert.equal(h.state.rpcCalls,0);assert.equal(h.state.itemWrites,1);assert.equal(h.state.supply_request_items[0].item_id,null)});
