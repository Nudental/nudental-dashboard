...
const source=fs.readFileSync(path.join(root,'src/pages/pending-approvals/index.jsx'),'utf8'),tree=parser.parse(source,{sourceType:'module',plugins:['jsx']});
...
function notifier({profile={email:address},lookupError=false,networkError=false,ok=true,body={success:true,id:'qa-mock-message'},invalidJson=false}={}){
 ...
 state.run=vm.runInNewContext(code('sendRejectionEmail'),{qaEnv:{VITE_SUPABASE_URL:'https://qa.example.invalid',VITE_SUPABASE_ANON_KEY:'qa-placeholder'},
...
for(const name of ['handleReject','handleRejectAfterApproval'])for(const accepted of [false,true])test(name+' reports saved state and '+(accepted?'accepted request':'unconfirmed notification'),async()=>{
 ...
 const ctx={supabase:{from(){return {update(){return query;}};}},userProfile:{id:'qa-reviewer',role:'regional_manager'},
  sendRejectionEmail:async()=>accepted,logStatusChange:async()=>{},success:(...m)=>messages.push(m),toastError(){assert.fail('Rejection state should still save');},
  rejectAfterApprovalReason:'QA reversal reason',rejectAfterApprovalEntry:{...entry,status:'approved'},
 };
 for(const setter of ['setActionLoading','setReviewEntry','setReviewNote','fetchEntries','fetchFullCounts','setShowRejectWarning','setRejectAfterApprovalEntry','setRejectAfterApprovalReason'])ctx[setter]=()=>{};
 const run=vm.runInNewContext(code(name),ctx);await run(entry,'QA reason');assert.equal(messages.length,1);
