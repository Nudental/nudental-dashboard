...
function harness(fail=false){let items=[{id:'qa',current_qty:5,min_required:5,status:'Low'}],summary={totalInStock:0,lowItems:1,criticalOutOfStock:0},fresh,reads=0;const errors=[],toasts=[];
 ...
 const ctx=vm.createContext({selectedOffice:'QA / Office A',selectedMonth:'',selectedCategory:'All',selectedStatus:'All',search:'',setLoading(){},setError:e=>{if(e)errors.push(e)},setItems:v=>items=typeof v==='function'?v(items):v,setSummary:v=>summary=v,showToast:t=>toasts.push(t),frontDeskInventoryService:{
 ...
 fetchInventory:async()=>database.map(i=>({...i})),fetchSummary:async()=>{reads++;fresh={totalInStock:database.filter(i=>i.status==='In Stock').length,lowItems:database.filter(i=>i.status==='Low').length,criticalOutOfStock:database.filter(i=>i.status==='Out of Stock').length};return fresh}
 }});for(const name of ['loadData','handleQtySave','handleRowSave','handleAddRow'])ctx[name]=vm.runInContext('('+code(name)+')',ctx);
 return{ctx,errors,toasts,get reads(){return reads},get summary(){return summary},get fresh(){return fresh},get items(){return items}};}
