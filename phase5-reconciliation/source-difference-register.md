...
| StatisticalSummary API helper | Optional call to bound helper in two places | Direct helper call | Need prove actual helper binding is always defined before classifying equivalent. |
| insurance-verify rendering | `jsxs` at one call | `jsx` at counterpart | Need validate actual React production runtime aliases; do not normalize arbitrary property names. |
