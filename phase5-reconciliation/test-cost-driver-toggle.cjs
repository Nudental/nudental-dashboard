...
const source=fs.readFileSync(path.join(root,'CostDriversManagement.jsx'),'utf8');
...
function setup({denied=false}={}){
 ...
 const toggle=vm.runInNewContext('('+source.slice(handler.start,handler.end)+')',{
  costDriversService:{async update(id,payload){assert.equal(id,'qa-temporary-category');if(denied)throw new Error('permission denied');updates.push({id,...payload})}},
...
test('Cost-driver deactivation consumes the table ID and false target state',async()=>{
...
test('Cost-driver activation consumes the table ID and true target state',async()=>{
...
test('Rejected cost-driver status change reports the error and does not reload as success',async()=>{
