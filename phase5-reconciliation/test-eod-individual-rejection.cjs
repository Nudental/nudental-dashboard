...
const fn=find(parser.parse(source,{sourceType:'module',plugins:['jsx']}),n=>n.type==='VariableDeclarator'&&n.id.name==='handleReject').init;
...
function reviewer(db) {
 ...
 outcome.approve=vm.runInNewContext(code,{
  ...
  sendRejectionEmail:async()=>outcome.notifications++,setActionLoading(){},setReviewEntry(){},setReviewNote(){},fetchEntries(){},fetchFullCounts(){},
...
test('two concurrent reviews produce one write, one history event and one success',async()=>{
 ...
 await Promise.all([first.approve(entry('pending'),'QA reason'),second.approve(entry('pending'),'QA reason')]);
...
for(const status of ['pending','pending_review','pending_reapproval'])test('valid '+status+' rejection records one correct event',async()=>{
 const db=database(status),r=reviewer(db);await r.approve(entry(status),'QA rejection note');
 assert.equal(db.writes,1);assert.equal(db.row.status,'rejected');assert.equal(db.row.approved_by,'qa-regional');
...
test('permission denial cannot produce history or success',async()=>{
 const db=database('pending',{denied:true}),r=reviewer(db);await r.approve(entry('pending'),'QA reason');
 assert.equal(db.writes,0);assert.equal(r.history.length,0);assert.equal(r.success.length,0);assert.equal(r.errors.length,1);
});
test('missing reason makes no request',async()=>{const db=database('pending'),r=reviewer(db);await r.approve(entry('pending'),'  ');assert.equal(db.requests,0);assert.equal(r.history.length,0);assert.equal(r.notifications,0);assert.equal(r.errors.length,1);});
