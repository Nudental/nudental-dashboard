...
const source=fs.readFileSync(path.join(root,'src/pages/implant-inventory-management/components/ImplantUsageLogTab.jsx'),'utf8');
...
for(const tz of ['America/New_York','America/Los_Angeles','UTC','Asia/Tokyo'])test('usage calendar dates stay unchanged in '+tz,()=>{
...
test('missing usage date remains an em dash',()=>{for(const value of [undefined,null,''])assert.equal(format(value),'—');});
