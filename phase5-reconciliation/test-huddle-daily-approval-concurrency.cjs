...
const fn=find(parser.parse(source,{sourceType:'module',plugins:['jsx']}),n=>n.type==='VariableDeclarator'&&n.id.name==='handleDRApprove').init;
...
function reviewer(db, reviewedStatus='pending') {
 ...
 outcome.approve=vm.runInNewContext(code,{
  ...
  drApproveTarget:entry(reviewedStatus),isDRActionable:e=>['pending','pending_reapproval'].includes(e.status),setDrActionLoading(){},setDrApproveTarget(){},fetchDailyReviews(){},
...
test('two concurrent reviews produce one write, one history event and one success',async()=>{
 ...
 await Promise.all([first.approve(),second.approve()]);
...
for(const status of ['pending','pending_reapproval'])test('valid '+status+' approval records one correct event',async()=>{
 const db=database(status),r=reviewer(db,status);await r.approve('QA approval note');
...
test('permission denial cannot produce history or success',async()=>{
 const db=database('pending',{denied:true}),r=reviewer(db);await r.approve();
...
test('an already-approved local record remains rejected before a request',async()=>{
 const db=database('approved'),r=reviewer(db,'approved');await r.approve();
